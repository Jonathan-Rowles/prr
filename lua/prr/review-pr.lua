...
function M.review_pr()
    vim.notify("Checking for open PRs...", vim.log.levels.INFO)
    ...
    pickers.new({}, {
        ...
        finder = finders.new_table({
            ...
            entry_maker = function(entry)
                return {
                    value = entry.branch,
                    display = string.format("[%s] %s", sanitize_line(entry.branch), sanitize_line(entry.title)),
                    ordinal = sanitize_line(entry.branch) .. " " .. sanitize_line(entry.title),
                    author = sanitize_line(entry.author),
                    createdAt = sanitize_line(entry.createdAt),
                    body = entry.body or "",
                    title = sanitize_line(entry.title),
                }
            end,
        }),
        ...
        previewer = previewers.new_buffer_previewer({
            ...
            define_preview = function(self, entry)
                local lines = {
                    "Branch: " .. sanitize_line(entry.value),
                    "Author: " .. sanitize_line(entry.author),
                    "Date Opened: " .. sanitize_line(entry.createdAt),
                    "",
                    "Title: " .. sanitize_line(entry.title),
                    "",
                    "Description:",
                ...
                if entry.body and entry.body ~= "" then
                    ...
                    for _, line in ipairs(body_lines) do
                        if type(lines) ~= "table" then
                            vim.notify("Error: 'lines' is not a table.", vim.log.levels.ERROR)
                            return
                        end
                        ...
                        if type(sanitized) ~= "string" then
                            vim.notify("Error: Sanitized line is not a string.", vim.log.levels.ERROR)
                            return
                        end
                ...
                if not success then
                    vim.notify("Error setting preview lines: " .. err, vim.log.levels.ERROR)
                    return
                end
